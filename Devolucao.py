...
def load_table():
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute("SELECT * FROM Devolucao")
...
def add_to_table_and_database(nota, rastreio):
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute("SELECT * FROM Devolucao WHERE Nota=%s", (nota,))
            ...
            sql_insert_query = """INSERT INTO Devolucao (Nota, Rastreio) 
                                  VALUES (%s, %s)"""
    ...
    if search_text:
        try:
            ...
            if connection.is_connected():
                ...
                cursor.execute("SELECT c.* FROM Devolucao c INNER JOIN lotes l ON c.Lote = l.id WHERE c.Nota LIKE %s OR c.Rastreio LIKE %s OR l.nome LIKE %s", ('%' + search_text + '%', '%' + search_text + '%', '%' + search_text + '%'))
                rows = cursor.fetchall()
    ...
    if selected_item:
        ...
        try:
            ...
            if connection.is_connected():
                ...
                cursor.execute("DELETE FROM Devolucao WHERE ID=%s", (item_id,))
                connection.commit()
    ...
    try:
        ...
        if connection.is_connected():
            ...
            if date == 'Todos os dias':
                ...
                cursor.execute("SELECT Nota, Rastreio, Date_added FROM Devolucao")
            else:
                # Selecionar todos os registros da tabela para o lote e data especificados
                cursor.execute("SELECT Nota, Rastreio, Date_added FROM Devolucao WHERE Lote IN (SELECT id FROM lotes WHERE nome = %s) AND DATE(Date_added) = %s", (lote_name, date))
            rows = cursor.fetchall()
            ...
            df.to_excel(f"{lote_name}_{date_str}_dados_Devolucao.xlsx", index=False)

            print(f"Dados exportados para o arquivo Excel: {lote_name}_{date_str}_dados_Devolucao.xlsx")
